...
class NetworkMapper(BaseModule):
    ...
    def build_content(self):
        self.add_title("Network Mapper", "Fast host discovery and network topology")
        ...
        tk.Label(sf, text="Target:", font=("Courier",10), fg="#fff", bg="#16213e").pack(anchor="w")
        ...
        tk.Button(bf, text="Ping Scan", font=("Courier",9), fg="#000", bg="#00ccff", relief="flat", padx=10, command=self._ping_scan).pack(side="left", padx=2)
        tk.Button(bf, text="Nmap Discover", font=("Courier",9), fg="#000", bg="#00ff88", relief="flat", padx=10, command=self._nmap_discover).pack(side="left", padx=2)
        ...
        tk.Button(bf, text="Stop", font=("Courier",9), fg="#fff", bg="#cc0000", relief="flat", padx=10, command=self._stop).pack(side="right", padx=2)
        self.map_frame = tk.Frame(self.inner, bg="#1a1a2e")
        self.map_frame.pack(fill="both", expand=True, padx=10, pady=5)
        self.status_label = self.add_status("Enter single IP or /24 range")
    ...
    def _ping_scan(self):
        ...
        self.hosts = []
        self.status_label.config(text="Scanning " + target + "...")
        def do():
            if "/" in target:
                ...
                for i in range(1, 255):
                    ...
                    try:
                        ...
                        if r.returncode == 0:
                            self.hosts.append({"ip":ip,"status":"up","method":"ping"})
                            self.frame.after(0, self._render_map)
                            self.frame.after(0, lambda c=len(self.hosts): self.status_label.config(text="Found " + str(c) + " hosts..."))
                    except: pass
            else:
                try:
                    r = subprocess.run(["ping","-c","1","-W","1",target], capture_output=True, timeout=2)
                    if r.returncode == 0:
                        self.hosts.append({"ip":target,"status":"up","method":"ping"})
                except: pass
                self.frame.after(0, self._render_map)
            self.frame.after(0, lambda: self.status_label.config(text="Done - " + str(len(self.hosts)) + " hosts"))
        threading.Thread(target=do, daemon=True).start()

    def _nmap_discover(self):
        target = self.target_entry.get().strip()
        if not target: return
        self.status_label.config(text="Nmap scanning " + target + "...")
        self.scan_running = True
        def do():
            try:
                r = subprocess.run(["nmap","-sn","-T4",target,"-oG","-"], capture_output=True, text=True, timeout=60)
                self.hosts = []
                for line in r.stdout.split("\n"):
                    if "Status: Up" in line:
                        parts = line.split()
                        if len(parts) > 1:
                            self.hosts.append({"ip":parts[1],"status":"up","method":"nmap"})
                self.frame.after(0, self._render_map)
                self.frame.after(0, lambda: self.status_label.config(text="Found " + str(len(self.hosts)) + " hosts"))
    ...
    def _arp_scan(self):
        ...
        def do():
            try:
                r = subprocess.run(["arp-scan","-l","-t","1000"], capture_output=True, text=True, timeout=30)
                self.hosts = []
                for line in r.stdout.split("\n"):
                    parts = line.split()
                    if len(parts) >= 2 and re.match(r"\d+\.\d+\.\d+\.\d+", parts[0]):
                        self.hosts.append({"ip":parts[0],"mac":parts[1],"method":"arp"})
                self.frame.after(0, self._render_map)
                self.frame.after(0, lambda: self.status_label.config(text="Found " + str(len(self.hosts)) + " hosts"))
            except:
    ...
    def _render_map(self):
        for w in self.map_frame.winfo_children(): w.destroy()
        if not self.hosts:
            tk.Label(self.map_frame, text="No hosts found yet...", font=("Courier",10), fg="#888", bg="#1a1a2e").pack(pady=20)
            return
        ...
        for i, host in enumerate(self.hosts):
            ...
            card.pack(side="left", padx=3, pady=3, fill="x", expand=True)
            tk.Label(card, text=host.get("ip","?"), font=("Courier",9,"bold"), fg="#3fb950", bg="#16213e").pack()
            ...
            tk.Button(card, text="Scan Ports", font=("Courier",7), fg="#000", bg="#00ccff", relief="flat", padx=4, command=lambda h=host: self._scan_host(h)).pack(pady=2)
