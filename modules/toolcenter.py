...
class ToolCenter:
    ...
    def build(self):
        ...
        cat_icons = {'recon':'🎯','web':'🌍','network':'🌐','credentials':'🔑','wireless':'📡','forensics':'🔍','programming':'💻','exploitation':'💣','hash_crypto':'#️⃣','stegano':'🖼️','dos':'💥','secure_delete':'🗑️','malware_analysis':'🦠','phishing_social':'🎣','osint':'🔎','wifi_bluetooth':'📶','web_exploit':'🕸️','android_hacking':'📱','voip':'📞','social_engineering':'🎭','crypto_stego':'🔐','reverse_engineering':'⚙️','api_testing':'🔌','advanced_c2':'☠️','evasion_bypass':'🛡️','red_team_infra':'🏗️','initial_access':'🚪','privilege_escalation':'⬆️','lateral_movement':'↔️','exfiltration':'📤','persistence':'📌','defense_evasion':'👻','credential_access':'💳','collection':'📥','impact_ransomware':'💀','cloud_attacks':'☁️','ai_ml_tools':'🤖','quantum_crypto':'⚛️','blockchain_web3':'⛓️','satellite_space':'🛰️','biometric_hack':'👁️'}
    ...
    def _category_section(self, parent, category, tools, icon, installed):
        ...
        tk.Button(header, text=f"{arrow} {icon} {category.upper()} ({count})", font=('Courier',10,'bold'),
    ...
    def _build_tool_list(self, parent, tools):
        for tool in sorted(tools, key=lambda t: t['name']):
            ...
            tk.Button(row, text=f"✅ {tool['name']}", font=('Courier',9), fg='#00ff88', bg='#16213e',
    ...
    def _build_missing_list(self, parent, tools):
        ...
        for t in tools: by_cat[t['category']].append(t['name'])
        ...
        tk.Label(parent, text=f"Environment: {env} | Click install button", font=('Courier',8), fg='#ffaa00', bg='#1a1a2e').pack(anchor='w')
    ...
    def _show_tool_detail(self, tool):
        d = tk.Toplevel(self.parent, bg='#1a1a2e'); d.title(tool['name']); d.geometry("500x400")
        tk.Label(d, text=f"✅ {tool['name']}", font=('Courier',14,'bold'), fg='#00ff88', bg='#1a1a2e').pack(pady=10)
        tk.Label(d, text=f"Command: {tool['command']}\nPath: {tool['path']}\nVersion: {tool.get('version','?')}", font=('Courier',9), fg='#aaa', bg='#1a1a2e').pack(pady=5)
        if self.args_db:
            args = self.args_db.get_args(tool['name'])
            if args:
                tk.Label(d, text="Quick Arguments:", font=('Courier',10,'bold'), fg='#ffaa00', bg='#1a1a2e').pack(pady=(10,5))
                for arg, desc in args[:12]:
                    f = tk.Frame(d, bg='#16213e'); f.pack(fill='x', pady=1)
                    tk.Label(f, text=arg[:20], font=('Courier',9), fg='#00ccff', bg='#16213e', width=22, anchor='w').pack(side='left', padx=5)
                    tk.Label(f, text=desc[:40], font=('Courier',8), fg='#888', bg='#16213e', anchor='w').pack(side='left', padx=5)
        bf = tk.Frame(d, bg='#1a1a2e'); bf.pack(pady=10)
    ...
    def _run_tool_dialog(self, tool):
        ...
        if self.args_db:
            args = self.args_db.get_args(tool['name'])
            if args:
                af = tk.Frame(d, bg='#1a1a2e'); af.pack(fill='x', padx=10)
                for arg, desc in args[:15]:
                    tk.Button(af, text=arg[:14], font=('Courier',7), fg='#000', bg='#0f3460', relief='flat',
                            command=lambda a=arg: args_entry.insert('end',' '+a)).pack(side='left', padx=1, pady=1)
        output = tk.Text(d, font=('Courier',9), bg='#0a0a0a', fg='#00ff88', relief='flat')
        output.pack(fill='both', expand=True, padx=10, pady=5)
        stop_flag = [False]
        def execute():
            a = args_entry.get().strip(); cmd = f"{tool['command']} {a}" if a else tool['command']
            output.insert('end', f"\n$ {cmd}\n{'='*40}\n\n"); output.see('end'); stop_flag[0] = False
            def run():
                try:
        ...
        tk.Button(bf2, text="Stop", font=('Courier',10), fg='#fff', bg='#cc0000', relief='raised', padx=15, pady=6, command=lambda: stop_flag.__setitem__(0,True)).pack(side='left', padx=3)
